import { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import api from '../../lib/axios';
import useAuthStore from '../../store/authStore';

function Card({ children, style = {} }) {
  return (
    <div style={{
      background: '#101F3A', border: '1px solid #1C3560',
      borderRadius: 12, padding: '20px 24px', ...style,
    }}>{children}</div>
  );
}

function SectionTitle({ children }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
      <div style={{ width: 3, height: 20, background: '#F0A500', borderRadius: 2 }} />
      <span style={{ color: '#E8EDF5', fontWeight: 700, fontSize: 15 }}>{children}</span>
    </div>
  );
}

function Badge({ statut }) {
  const map = {
    'planifiee': { bg: '#1A3A2A', color: '#22C55E' },
    'en_cours':  { bg: '#1D3F7A', color: '#60A5FA' },
    'terminee':  { bg: '#2A2A1A', color: '#F59E0B' },
    'annulee':   { bg: '#3A1A1A', color: '#EF4444' },
    'haute':     { bg: '#3A1A1A', color: '#EF4444' },
    'normale':   { bg: '#1D3F7A', color: '#60A5FA' },
    'basse':     { bg: '#1A3A2A', color: '#22C55E' },
  };
  const s = map[statut] || { bg: '#222', color: '#aaa' };
  return (
    <span style={{
      background: s.bg, color: s.color,
      padding: '2px 10px', borderRadius: 20,
      fontSize: 12, fontWeight: 600,
    }}>{statut?.replace('_', ' ')}</span>
  );
}

export default function DashboardClient() {
  const { user }  = useAuthStore();
  const navigate  = useNavigate();
  const [data, setData]       = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError]     = useState(null);

  useEffect(() => { fetchData(); }, []);

  const fetchData = async () => {
    try {
      const res = await api.get('/client/dashboard');
      setData(res.data);
    } catch {
      setError('Impossible de charger le tableau de bord.');
    } finally {
      setLoading(false);
    }
  };

  if (loading) return (
    <div style={{ color: '#6B84AA', textAlign: 'center', padding: '60px' }}>Chargement...</div>
  );

  if (error) return (
    <div style={{
      background: '#3A1A1A', border: '1px solid #EF4444',
      borderRadius: 12, padding: '20px', color: '#EF4444',
    }}>{error}</div>
  );

  const { client, stats, contrats_expirants, interventions_recentes } = data;

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>

      {/* Bienvenue */}
      <div style={{
        background: 'linear-gradient(135deg, #0F1E35, #1D6FA4)',
        borderRadius: 14, padding: '24px 28px',
        display: 'flex', alignItems: 'center', gap: 18,
      }}>
        <div style={{
          width: 60, height: 60, borderRadius: 14,
          background: 'linear-gradient(135deg, #F0A500, #FFD166)',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontWeight: 800, fontSize: 22, color: '#0F1E35', flexShrink: 0,
        }}>
          {client?.raison_sociale?.[0]}
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ fontWeight: 800, fontSize: 20, color: '#fff' }}>
            {client?.raison_sociale}
          </div>
          <div style={{ color: '#A0B4CC', fontSize: 13, marginTop: 2 }}>
            {client?.secteur} · {client?.ville}
          </div>
          <div style={{ marginTop: 8 }}>
            <span style={{
              background: '#22C55E22', color: '#22C55E',
              padding: '3px 12px', borderRadius: 20, fontSize: 12, fontWeight: 700,
            }}>{client?.statut}</span>
          </div>
        </div>
        <div style={{ textAlign: 'right', color: '#A0B4CC', fontSize: 13 }}>
          {new Date().toLocaleDateString('fr-FR', {
            weekday: 'long', day: 'numeric', month: 'long', year: 'numeric'
          })}
        </div>
      </div>

      {/* Stats */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
        {[
          { label: 'Contrats actifs',       value: stats.contrats_actifs,          color: '#1D6FA4', icon: '📋',
            delta: stats.contrats_expirant > 0 ? `⚠ ${stats.contrats_expirant} expirent bientôt` : null },
          { label: 'Interventions en cours', value: stats.interventions_en_cours,   color: '#60A5FA', icon: '🔧',
            delta: `${stats.interventions_planifiees} planifiées` },
          { label: 'Interventions terminées',value: stats.interventions_terminees,  color: '#22C55E', icon: '✅',
            delta: `sur ${stats.interventions_total} total` },
          { label: 'Équipements',            value: stats.equipements_total,        color: '#A855F7', icon: '💻',
            delta: stats.equipements_hors_service > 0 ? `⚠ ${stats.equipements_hors_service} hors service` : '✅ Tous opérationnels' },
        ].map((s, i) => (
          <Card key={i} style={{ position: 'relative', overflow: 'hidden' }}>
            <div style={{
              position: 'absolute', top: -20, right: -20,
              width: 80, height: 80, borderRadius: '50%',
              background: s.color + '18',
            }} />
            <div style={{ fontSize: 28, marginBottom: 8 }}>{s.icon}</div>
            <div style={{ fontSize: 32, fontWeight: 800, color: s.color, lineHeight: 1 }}>{s.value}</div>
            <div style={{ fontSize: 13, color: '#6B84AA', marginTop: 4 }}>{s.label}</div>
            {s.delta && (
              <div style={{ fontSize: 12, color: s.color, marginTop: 6, fontWeight: 600 }}>{s.delta}</div>
            )}
          </Card>
        ))}
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>

        {/* Contrats expirants */}
        <Card>
          <SectionTitle>⚠ Contrats expirant bientôt</SectionTitle>
          {contrats_expirants.length === 0 ? (
            <div style={{ color: '#6B84AA', textAlign: 'center', padding: '30px 0', fontSize: 13 }}>
              ✅ Aucun contrat n'expire dans les 30 prochains jours
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {contrats_expirants.map((c, i) => (
                <div key={i} style={{
                  background: c.jours_restants <= 7 ? '#3A1A1A' : '#1A2A3A',
                  borderRadius: 8, padding: '12px 14px',
                  borderLeft: `3px solid ${c.jours_restants <= 7 ? '#EF4444' : '#F59E0B'}`,
                  display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                }}>
                  <div>
                    <div style={{ color: '#E8EDF5', fontWeight: 600, fontSize: 13 }}>{c.reference}</div>
                    <div style={{ color: '#6B84AA', fontSize: 12, marginTop: 2 }}>
                      {c.type_contrat?.replace(/_/g, ' ')} · Fin : {c.date_fin}
                    </div>
                  </div>
                  <div style={{
                    color: c.jours_restants <= 7 ? '#EF4444' : '#F59E0B',
                    fontWeight: 800, fontSize: 16,
                  }}>J-{c.jours_restants}</div>
                </div>
              ))}
            </div>
          )}
          <button onClick={() => navigate('/app/client/contrats')} style={{
            marginTop: 14, width: '100%',
            background: 'transparent', border: '1px solid #1C3560',
            borderRadius: 8, padding: '8px',
            color: '#6B84AA', fontSize: 13, cursor: 'pointer',
          }}>Voir tous mes contrats →</button>
        </Card>

        {/* Interventions récentes */}
        <Card>
          <SectionTitle>Interventions récentes</SectionTitle>
          {interventions_recentes.length === 0 ? (
            <div style={{ color: '#6B84AA', textAlign: 'center', padding: '30px 0', fontSize: 13 }}>
              Aucune intervention enregistrée
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {interventions_recentes.map((inv, i) => (
                <div key={i} style={{
                  background: '#0D1F3C', borderRadius: 8, padding: '12px 14px',
                  borderLeft: `3px solid ${inv.statut === 'en_cours' ? '#60A5FA' : inv.statut === 'terminee' ? '#F59E0B' : '#22C55E'}`,
                  display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                }}>
                  <div>
                    <div style={{ color: '#2589C8', fontWeight: 700, fontSize: 13 }}>{inv.reference}</div>
                    <div style={{ color: '#6B84AA', fontSize: 12, marginTop: 2 }}>
                      {inv.type_intervention} · {new Date(inv.date_planifiee).toLocaleDateString('fr-FR')}
                    </div>
                  </div>
                  <Badge statut={inv.statut} />
                </div>
              ))}
            </div>
          )}
          <button onClick={() => navigate('/app/interventions')} style={{
            marginTop: 14, width: '100%',
            background: 'transparent', border: '1px solid #1C3560',
            borderRadius: 8, padding: '8px',
            color: '#6B84AA', fontSize: 13, cursor: 'pointer',
          }}>Voir toutes mes interventions →</button>
        </Card>
      </div>
    </div>
  );
}
